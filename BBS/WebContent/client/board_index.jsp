...
<script type="text/javascript">
	function validate() {
		...
		if (page > <s:property value="#session.pageBean.totalPage"/>) {
			...
			window.document.location.href = "indexBoardAction";
	...
	<div class="page-container">
		<div class="container">
			<div class="row">
				...
				<div class="span page-content">
					...
					<div class="row separator">
						...
						<s:iterator value="#session.topPosts" id="topPosts">
							<li class="article-entry standard a">
								<h4>
									<span style="color: red;">[置顶] </span><a href="postAction?postId=<s:property value = "#topPosts.postId" />"><s:property value="#topPosts.title" /></a>
								</h4> <span class="article-meta"><s:property value="#topPosts.content" /><span
									style="float: right; margin-right: 70px; margin-top:-20px;"><a href="#" style="margin-right:10px;"><s:property value="#topPosts.postPeople" />
									</a><s:property value="#topPosts.publishTime" /></span></span> <span class="like-count">回帖数：<span><s:property value="#topPosts.replyTime" /></span></span>
							</li>
						</s:iterator>
							<s:iterator value="#session.pageBean.list" id="posts">
								<li class="article-entry standard a">
									<h4>
										<a href="postAction?postId=<s:property value = "#posts.postId" /> "><s:property value="#posts.title" /> </a>
						...
						<div style="float: right; margin-top: 10px;">
							<center style="float: right;">
								...
								<form action="indexBoardAction?boardId=<s:property value="#session.detailBoardInfo.boardId" />" onsubmit="return validate();">
            ...
            首页&nbsp;&nbsp;&nbsp;上一页
								...
								<s:else>
									<a href="indexBoardAction?boardId=<s:property value="#session.detailBoardInfo.boardId" />">首页</a>
            &nbsp;&nbsp;&nbsp;
            <a
										href="indexBoardAction?boardId=<s:property value="#session.detailBoardInfo.boardId" />&page=<s:property value="#session.pageBean.currentPage - 1"/>">上一页</a>
								</s:else>

								<s:if
									test="#session.pageBean.currentPage != #session.pageBean.totalPage">
									<a
										href="indexBoardAction?boardId=<s:property value="#session.detailBoardInfo.boardId" />&page=<s:property value="#session.pageBean.currentPage + 1"/>">下一页</a>
            &nbsp;&nbsp;&nbsp;
            <a
										href="indexBoardAction?boardId=<s:property value="#session.detailBoardInfo.boardId" />&page=<s:property value="#session.pageBean.totalPage"/>">尾页</a>
								</s:if>
            ...
            下一页&nbsp;&nbsp;&nbsp;尾页
						...
						<div>
							...
							<div style="float: left; width: 600px;">
								主题：<input type="text"
									style="border: 1px solid gainsboro; width: 500px;" name="title"/>
							</div>
							<div style="float: left; width: 600px;" >
								内容：
								<textarea
									style="border: 1px solid gainsboro; width: 500px; height: 100px;"
									rows="5" cols="20" name="content"></textarea>
								<input style="margin: 0 50px; width: 60px;" type="submit"
									value="发布" />
							</div>
